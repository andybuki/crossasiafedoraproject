...
public class MinguoBookLanguage {
    public static void main(String[] args) throws IOException {
        PrintStream out = new PrintStream(new FileOutputStream("/data3/solr/minguo/books_language_solr.json"));
        String books = "/data3/solr/minguo/books.json";
        String language = "/data3/solr/minguo/language.json";
        String quote = "\u005c\u0022";
        JSONArray booksObject = new JSONArray(new JSONTokener(new FileInputStream(books)));
        JSONArray languageObject = new JSONArray(new JSONTokener(new FileInputStream(language)));
        ...
        for (int i=0; i<booksObject.length(); i++) {
            ...
            for (int j=0; j<languageObject.length(); j++) {
                JSONObject languageObj = (JSONObject) languageObject.get(j);
                String id_lan="";
                JSONArray language_lan = null;
                id_lan = (String) languageObj.get("id").toString();
                if (languageObj.has("language"))
                    language_lan =(JSONArray) languageObj.get("language");
                ...
                if (id.equals(id_lan)) {
                    ...
                    if (language!=null)
                        sb.append(  quote + "language" + quote + ":" +  language_lan+  "," + '\n');
                    ...
                    sb.append(          "{"+language_lan.toString().replace("[","").replace("]","")+"}"+ "|");
                    sb.append(          book + "|");
